...
| PERF-056 | CSS background serves the original while a WebP sibling exists | Grep the theme's templates and CSS for `background-image` / `background:` with a `url(...)` pointing at `wp-content/uploads`. For each, test both sibling names on disk — `<file>.webp` (what Robin Image Optimizer and most bulk optimizers write) and `<name>.webp` (what WordPress writes). Report every URL whose sibling exists but whose declaration names only the original, and whose value is not produced by the theme's `prefix_background_image()` helper | WARNING | Templates only — a background in a stylesheet needs the server rule (see the `wp-robin` skill), not a template edit |
