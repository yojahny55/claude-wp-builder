...
- **Grep the demo section for `MOCK:` and `data-mock` first.** The demo-authoring tooling
  ...
  come from `get_terms()` on the records this section queries — see the option-set rule in
  the section on control option sets, if this repo's Rule 12 for that is present, or apply
  the same principle directly: build options from real data, never from the demo's `<option>`
  values), or drop the control and say so in your report. Shipping a search box that posts
...
11. **Date-ordered archive/directory queries add `ID` as a tiebreaker** — `'orderby' => array('date' => 'DESC', 'ID' => 'DESC')` — seeded records routinely share a `post_date` to the second
12. **A custom nav walker overriding `start_el()` re-applies `nav_menu_css_class`, `nav_menu_item_id` and `nav_menu_link_attributes`** via `apply_filters()` — overriding the method replaces core's own calls to them
13. **Grep the demo section for `MOCK:` / `data-mock` before wiring any control to real data** — re-derive it from the real query or drop it and say so; never ship a search that posts nowhere, a pager that pages nothing, or a filter checked against values the real cards don't carry
14. **An optional CMS "see more" URL control renders only when its field holds a real `http(s)://` or site-relative URL** — never for a demo `#anchor` placeholder — and never hardcode an example person/address/office in the template for a relation that has not been seeded yet
...
16. **Reuse a sibling template's already-solved interactive pattern** (accordion chevron via the trigger button's `aria-expanded` group/peer variant, a focus ring on the whole control box) instead of re-deriving a fresh implementation per section
