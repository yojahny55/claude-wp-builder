...
14. **Never give an options-page field a `default_value`.** A default is not an empty value, so any per-language layer that falls back "when the bucket is empty" — `acf-options-for-polylang` is the usual one — never falls back at all: the seeded value, and every later edit the client makes, is invisible on the front end while the default renders. Options-page defaults also duplicate a fallback the template almost always already has. Put the fallback in the template (`cubasol_get_field('x','option') ?: 'Default'`), never in the field definition.
