...
**banned as a default.** It is the house style of AI-built sites, it announces
that nothing on the page is real, and for a business selling an actual service it
works against the sale. A clay render of a document does not make anyone trust a
firm.

Use an illustrated world only when the brand is genuinely illustrated — a
children's product, a game, a brand whose existing identity is drawn — and then
commit to it properly, matched to the brand's real illustration style rather than
to the generic diorama look.

## The eight preambles

Each is a starting preamble, ready to copy. Tune the light, lens and grade to the
brand; keep the structure.

### 1. Low-key cinematic — the default
Dark, controlled, one light source. Almost anything premium: spirits, coffee,
tools, apparel, software, professional services, finance.
...
anything with a process worth drawing.
...
1. **Medium and lens** — "35mm anamorphic", "100mm macro", "handheld 35mm". This
   is what sets depth of field and perspective.
...
after the fact. This is also why `bin/image-gen.mjs` resolves a crop per slot:
the crop is a delivery decision, not a composition one.

## If the canvas is light

Every worked example above is dark, and a high-key build inverts several things
at once:

1. **The scrim washes toward the canvas, it does not darken.** Over media on a
   paper ground the density goes *up*, or the type has nothing to sit on.
2. **Type over media stays ink.** Light text on a light page over a bright frame
   is unrecoverable.
3. **Inset edge highlights are tuned for a near-black ground.** On paper they
   read as dirt along the top lip. Invert to white, or drop them and carry the
   raise with a hairline.
4. **Shadow alphas are tuned for a near-black ground** too. Halve them and
   re-tint the shadow colour toward the canvas hue.

Contrast direction flips with all of this: dark type fails on the *darkest* patch
under it, not the brightest.
