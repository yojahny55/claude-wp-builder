...
it does not accept, a failed query, or an uploads directory `wp_get_upload_dir()` cannot resolve.
...
counts every miss and prints a sample per bucket (20 by default), never the whole list.
