...
`<home-host>` is `wordpress.url` from `.wp-create.json` (or `$WP option get home`). Exit codes: `0` = report returned, `1` = tool error, `2` = skipped (no `curl`, no network, no report, or a transient `429`/`503`). Exit `2` is a clean skip — neither a success nor a failure: report the live score as unavailable and continue. A localhost or otherwise non-public URL legitimately yields exit `2`, because the scan needs a publicly reachable host; record it as a skip and mark the run incomplete. On exit `1`, record the error and continue. Only a returned report yields a score; map its failed ORA check ids back to GEO codes using the `wp-audit-geo-standards` skill. Advisory and off-site findings (GEO-D05 through GEO-D08, GEO-U10, GEO-P01 through GEO-P05) are left unfixed.
