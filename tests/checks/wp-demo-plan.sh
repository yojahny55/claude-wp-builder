...
# Anchor on the capture PROCEDURE, not on a sentence that happens to list the
# key names: the plan's own "carries none of those" line enumerates all three,
# so a looser pattern passes even with the Fidelity capture section deleted.
grep -Fq 'section.backgrounds[]' "$n" || { echo "FAIL: wp-normalize no longer captures backgrounds"; exit 1; }
grep -Fq '@font-face' "$n" || { echo "FAIL: wp-normalize no longer captures fonts"; exit 1; }
grep -Fq 'section.cssRules' "$n" || { echo "FAIL: wp-normalize no longer captures verbatim CSS"; exit 1; }
