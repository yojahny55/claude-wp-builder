...
grep -Fq 'set `$WP` to `wp`' "$f" || fail "$f does not fall back to \$WP=wp"
...
grep -Fq 'fall back to a bare `wp`' README.md || fail "README still tells the reader /wp-seed needs .wp-create.json"
