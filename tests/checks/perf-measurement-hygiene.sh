...
printf '%s' "$flats" | grep -Fq 'Re-measure before filing a metric regression' \
...
printf '%s' "$flats" | grep -Fq 'both halves must be measured under the' \
...
printf '%s' "$flats" | grep -Fq 'the LCP number decides' \
